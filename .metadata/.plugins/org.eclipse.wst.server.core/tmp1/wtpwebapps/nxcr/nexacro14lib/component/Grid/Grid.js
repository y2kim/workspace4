...
if (!nexacro.Grid) {
	...
	_pGridCell._hideEditor = function () {
		...
		this._grid._currentCellCol = -1;
		this._grid._currentCellSubRow = -1;

		if (this._subComp) {
			if (this._subComp._displaymode) {
				this._grid._changeDisplayer = true;
				this._updateDisplayer();
				this._subComp._updateAll();
				this._grid._changeDisplayer = false;
			}
			else {
	...
	_pGrid._setSelectBorder = function (selectborder, is_recreate) {
		...
		if (band && !selectborder) {
			selectborder = band.on_find_CurrentStyle_selectborder(band._pseudo);
		}
