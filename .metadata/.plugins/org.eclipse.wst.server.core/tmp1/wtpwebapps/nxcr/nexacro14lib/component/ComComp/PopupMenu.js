	...
	_pPopupMenu.on_mouseleave_basic_action = function (is_subcontrol_bubble) {
		if (is_subcontrol_bubble) {
			if (this._apply_pushed_pseudo && this._is_push) {
				...
				var _flag = this._items && (this._previousitem != -1) && (this._previousitem < this._items.length);

				var _curritem = _flag ? this._items[this._previousitem] : null;
				var _rootitem = this._getRootComponent(this)._menuitemonmouseenter;

				if (_curritem && _rootitem) {
					if (_curritem != _rootitem) {
						while (_rootitem) {
							if (this == _rootitem) {
							...
							_rootitem = _rootitem.parent;
						}
					}
					else if (_rootitem.parent._popupmenu) {
						var _items = _rootitem.parent._popupmenu._items;
						var _index = _rootitem.parent._popupmenu._previousitem;

						_flag = _items && (_index != -1) && (_index < _items.length);

						if (_flag && _items[_index] && (_items[_index]._pseudo == "mouseover")) {
							_curritem._stat_change("enable", "mouseover");
						}
						else if (_rootitem.parent._popupmenu && _rootitem.parent._popupmenu._is_popup()) {
							_curritem._stat_change("enable", "mouseover");
						}
						_items = null;
						_index = null;
